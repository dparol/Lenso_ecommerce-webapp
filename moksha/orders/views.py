...
import datetime
from .models import Order, Payment, OrderProduct,OrderStatus
import json
...
def place_order(request, total=0, quantity=0):
    ...
    if  request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if payment_type == data.payment_method:
...
def paymenthandler(request, total=0, quantity=0):
    ...
    if request.method == 'POST':
            ...
            if result is not None:
                ...
                print(param)
                # Capture the payment
                return redirect('order_complete/?' + param)
...
def order_status(request,id):
    ...
    # user_detail=Order.objects.filter(user=current_user,is_ordered=True)
    status = Order.objects.get(id=id)
    order = Order.objects.get(id=status.order.id)
    order_number=order.order_number
    ...
    return render(request,'mkhome/order_status.html',context)
